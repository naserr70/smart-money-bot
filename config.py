...
DEFAULT_EXCHANGE_WALLETS: Dict[str, Dict[str, str]] = {
    "ETH": {"0x28c6c06298d514db089934071355e5743bf21d60": "Binance 14", "0xf977814e90da44bfa03b6295a0616a897441acec": "Binance Hot Wallet 20"},
    "BSC": {}, "TRON": {},
}
...
def _load_exchange_wallets() -> Dict[str, Dict[str, str]]:
    ...
    if not raw: return merged
    try: parsed = json.loads(raw)
    except (json.JSONDecodeError, TypeError): return merged
    if not isinstance(parsed, dict): return merged
    for chain, addresses in parsed.items():
        if isinstance(addresses, dict): merged.setdefault(str(chain).upper(), {}).update({str(k): str(v) for k, v in addresses.items()})
    return merged
...
class Settings:
    ...
    github_max_files_per_sync: int = field(default_factory=lambda: _env_int("GITHUB_MAX_FILES_PER_SYNC", 0))
    ...
    @property
    def history_window(self) -> int: return self.candle_history_limit
    @property
    def history_candle_limit(self) -> int: return self.candle_history_limit
    @property
    def github_candle_store_enabled(self) -> bool: return bool(self.github_token and self.github_repo)
    @property
    def github_candle_store_token(self) -> str: return self.github_token
    @property
    def github_candle_store_repo(self) -> str: return self.github_repo
    @property
    def github_candle_store_branch(self) -> str: return self.github_branch
    @property
    def admin_chat_id_resolved(self) -> str: return self.admin_chat_id or self.chat_id

    def enabled_market_sources(self) -> List[str]:
        result = []
        if self.binance_enabled: result.append("binance")
        if self.bybit_enabled: result.append("bybit")
        if self.kucoin_enabled: result.append("kucoin")
        return result

    def validate(self) -> List[str]:
        problems: List[str] = []
        if not self.bot_token: problems.append("BOT_TOKEN تنظیم نشده است.")
        if not self.admin_chat_id_resolved: problems.append("ADMIN_CHAT_ID/CHAT_ID تنظیم نشده است.")
        if not self.bot_access_password: problems.append("BOT_ACCESS_PASSWORD تنظیم نشده است.")
        if not self.telegram_webhook_secret: problems.append("TELEGRAM_WEBHOOK_SECRET تنظیم نشده است؛ webhook امن نیست.")
        if not self.github_token: problems.append("GITHUB_TOKEN تنظیم نشده است؛ persistence ابری کندل‌ها غیرفعال است.")
        if self.preferred_market_source not in {"binance", "bybit", "kucoin"}: problems.append("PREFERRED_MARKET_SOURCE نامعتبر است.")
        if not self.enabled_market_sources(): problems.append("هیچ market source فعالی وجود ندارد.")
        if self.candle_history_limit < max(self.volume_baseline_candles + 1, self.pump_min_history_candles): problems.append("CANDLE_HISTORY_LIMIT برای baseline فعلی کافی نیست.")
        if self.candle_history_limit <= 0: problems.append("CANDLE_HISTORY_LIMIT باید مثبت باشد.")
        if self.candle_interval != "5m": problems.append("CANDLE_INTERVAL باید 5m باشد.")
        if self.volume_baseline_candles < 1 or self.volume_baseline_candles >= self.candle_history_limit: problems.append("VOLUME_BASELINE_CANDLES نامعتبر است.")
        if self.pump_history_candles < 2: problems.append("PUMP_HISTORY_CANDLES باید حداقل 2 باشد.")
        if self.volume_spike_ratio < 1 or self.volume_signal_multiplier < 1: problems.append("نسبت حجم نمی‌تواند کمتر از 1 باشد.")
        if self.price_pump_min < 0 or self.price_pump_min >= self.price_pump_max: problems.append("PRICE_PUMP_MIN/MAX نامعتبر است.")
        if self.pump_zscore_threshold <= 0: problems.append("PUMP_ZSCORE_THRESHOLD باید مثبت باشد.")
        if self.scan_interval_sec <= 0 or self.whale_scan_interval_sec <= 0: problems.append("intervalهای scan باید مثبت باشند.")
        if self.github_candle_sync_interval_sec < 300: problems.append("GITHUB_CANDLE_SYNC_INTERVAL_SEC کمتر از 300 ثانیه است.")
        return problems
